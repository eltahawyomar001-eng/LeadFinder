...
export async function searchElevenEighty(
  categoryQuery: string,
  cityName: string
): Promise<Lead[]> {
  try {
    const firstWord = categoryQuery.split(' ')[0];
    const url = `https://www.11880.com/suche/${encodeURIComponent(firstWord)}/${encodeURIComponent(cityName)}`;
    ...
    for (const entry of entries) {
      ...
      leads.push({
        place_id: `e80_${Buffer.from(name + address).toString('base64').slice(0, 20)}`,
        name,
        phone,
        whatsapp_link,
