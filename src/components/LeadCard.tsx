...
export default function LeadCard({ lead, index, onViewMessage, onEmailFound, contacted, onContacted }: Props) {
  ...
  // Add to CRM: sends step 1 + schedules follow-ups
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        {lead.email && (
          <>
            ...
            {inCrm || contacted ? (
              ...
              <button
                ...
                style={{
                  ...
                  color: '#fff', borderRadius: '12px', padding: '13px 16px',
            ...
            {crmError && (
              <p style={{ color: '#f87171', fontSize: '12px', margin: '0', textAlign: 'center' }}>{crmError}</p>
            )}

            {/* Quick send (no tracking) */}
            {!inCrm && !contacted && (
              <>
                <button
                  ...
                  disabled={sending}
                  style={{
                    width: '100%', backgroundColor: 'transparent', border: '1px solid #1e293b',
                    color: '#475569', borderRadius: '10px', padding: '9px 16px',
                    fontSize: '12px', fontWeight: 600,
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                    cursor: sending ? 'not-allowed' : 'pointer',
                  }}
                >
                  {sending ? 'Sending...' : 'Send only (no CRM tracking)'}
                </button>
                {sendError && (
                  <p style={{ color: '#f87171', fontSize: '12px', margin: '0', textAlign: 'center' }}>{sendError}</p>
                )}
                <a href={mailtoHref} style={{ color: '#334155', fontSize: '11px', textAlign: 'center', textDecoration: 'underline' }}>
        ...
        <button
          ...
          style={{
            ...
            color: '#93c5fd', borderRadius: '12px', padding: '12px 16px',
